...
const getStyleOptions = (t: any): StyleOption[] => [
  { id: 'ghibli', name: t('styles.ghibli.name'), emoji: '🌟', description: t('styles.ghibli.description'), thumbnail: '/api/placeholder/120/120' },
  { id: 'dragonball', name: t('styles.dragonball.name'), emoji: '🐉', description: t('styles.dragonball.description'), thumbnail: '/api/placeholder/120/120' },
  { id: 'pixel', name: t('styles.pixel.name'), emoji: '🎮', description: t('styles.pixel.description'), thumbnail: '/api/placeholder/120/120' },
  { id: 'oil', name: t('styles.oil.name'), emoji: '🎨', description: t('styles.oil.description'), thumbnail: '/api/placeholder/120/120' },
  { id: 'cartoon', name: t('styles.cartoon.name'), emoji: '🎪', description: t('styles.cartoon.description'), thumbnail: '/api/placeholder/120/120' },
]
    ...
    const interval = setInterval(() => {
      setGenerationProgress((prev) => {
        if (prev >= 100) {
          ...
          setResultImageUrl('/api/placeholder/400/400')
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center">
                ...
                <span className="ml-3 text-sm text-slate-300 hidden sm:inline font-medium">{t('brandSubtitle')}</span>
              </div>

              {/* Desktop Navigation */}
              <nav className="hidden lg:flex space-x-8">
                <a href="#" className="text-slate-300 hover:text-white transition-all duration-300 hover:bg-white/10 px-3 py-2 rounded-lg">
                  {t('nav.home')}
                </a>
                <a href="#" className="text-slate-300 hover:text-white transition-all duration-300 hover:bg-white/10 px-3 py-2 rounded-lg">
                  {t('nav.features')}
                ...
                <div className="flex flex-col space-y-4">
                  ...
                  <div className="flex items-center justify-between pt-4">
                    <div className="flex space-x-3">
                      <button className="text-slate-300 hover:text-white transition-all duration-300">{t('nav.login')}</button>
            ...
            <div className="max-w-4xl mx-auto text-center">
              <div className="mb-16">
                ...
                <div className="flex flex-wrap justify-center gap-2 mb-8">
                  <span className="px-4 py-2 glass-dark rounded-full text-sm font-medium text-white border border-white/20">✨ {t('nav.features')}</span>
                  <span className="px-4 py-2 glass-dark rounded-full text-sm font-medium text-white border border-white/20">🎨 AI {t('brandSubtitle')}</span>
                  <span className="px-4 py-2 glass-dark rounded-full text-sm font-medium text-white border border-white/20">🚀 {t('buttons.freeTrial')}</span>
                </div>
              </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {styleOptions.map((style) => (
                      ...
                      >
                        ...
                        <div className="relative z-10">
                          <div className="text-4xl mb-3 group-hover:scale-110 transition-transform duration-300">{style.emoji}</div>
                          <h4 className="font-bold text-white mb-2 text-lg">{style.name}</h4>
                  ...
                  >
                    <span className="flex items-center justify-center gap-3">
                      ...
                      {selectedStyle && <span className="group-hover:translate-x-1 transition-transform duration-300">🚀</span>}
                    </span>
                  </button>
                </div>
              </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid lg:grid-cols-3 gap-12 items-center mb-12">
                ...
                <div className="space-y-6">
                  <h3 className="text-2xl font-bold text-white text-center font-display">{t('originalImage')}</h3>
                  {uploadedImageUrl && (
                    <div className="rounded-3xl overflow-hidden glass-dark border border-white/20 shadow-card group">
                      <img
                        src={uploadedImageUrl}
                ...
                <div className="text-center">
                  <div className="text-6xl bg-gradient-primary bg-clip-text text-transparent animate-pulse">→</div>
                </div>

                {/* Result Image */}
                <div className="space-y-6">
                  <h3 className="text-2xl font-bold text-white text-center font-display">{t('generatedResult')}</h3>
                  {resultImageUrl && (
